...
The `overlay` slot receives the live `video` element — pass it to `<BarcodeDetectorOverlay :source="video">` (or your own SVG) and the overlay tracks the visible video rect automatically.
...
| `rejected` | `DetectedBarcode[]`        | Barcodes the `accept` predicate filtered out.                        |
| `video`    | `HTMLVideoElement \| null` | The internal video element. Pass to `<BarcodeDetectorOverlay :source="video">` to keep the auto-tracked alignment. |
...
<template>
  <UseBarcodeDetector>
    <template #overlay="{ detected, video }">
      <BarcodeDetectorOverlay :detected="detected" :source="video" stroke="hotpink" />
