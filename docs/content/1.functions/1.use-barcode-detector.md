...
Stop the camera as soon as a barcode is found. Pass a predicate to keep scanning until a matching barcode shows up. Call `start()` from a user gesture to begin (and again to re-arm after a stop).
