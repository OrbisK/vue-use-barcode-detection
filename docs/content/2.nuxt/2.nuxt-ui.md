...
When [`@nuxt/ui`](https://ui.nuxt.com) is present in your app, the [Nuxt module](/nuxt/module) registers an extra component built on top of `UInput`, `UButton`, and `UModal`.

## `<UBarcodeInput />`

A `UInput` with the scan trigger tucked into its `#trailing` slot, bound via `v-model`. When the [Barcode Detection API](https://developer.mozilla.org/docs/Web/API/Barcode_Detection_API) is available, tapping the trailing icon opens a modal with a live camera scanner; the first accepted barcode is written back to the model. The modal renders a `<BarcodeDetectorOverlay>` over the camera preview, so users see polygons over barcodes in real time — green for accepted matches, red for ones the `accept` predicate is filtering out. On unsupported browsers the scan button is hidden and the field behaves as a plain `UInput`.
